...
public class LibraryApp {
    ...
    private static String ERROR_NO_ACTIVE_ORDERS = "Tento zákazník nemá žádné aktivní výpůjčky";
...
//                            user have no debts
        ...
        givebackbookButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    HttpClient client = new DefaultHttpClient();
                    HttpGet get = new HttpGet("http://localhost:8090/getorderdetail/order/"+idfield.getText());
                    HttpResponse response = client.execute(get);
                    BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
                    String line = "";
                    while ((line = rd.readLine()) != null) {
                        try {
                            orderDetailDto = mapper.readValue(line,OrderDetailDto.class);
                            isEmpty = false;
                        } catch (UnrecognizedPropertyException e2){
//                            user have no active orders
                            isEmpty = true;
                        }
                    }

                    if (isEmpty){
                        showMessage(ERROR_NO_ACTIVE_ORDERS);
                    } else {
                        popUpOrderDetail(orderDetailDto);
                    }
    ...
    private void  popUpOrderDetail(OrderDetailDto orderDetailDto) {
        Object[] options = {"Vratit", "Zpět"};
        int n = JOptionPane.showOptionDialog(
                new JFrame(),
                orderDetailDto.toString(),
                "Debts",
                JOptionPane.YES_NO_CANCEL_OPTION,
                JOptionPane.QUESTION_MESSAGE,
                null,
